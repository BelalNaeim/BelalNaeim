...
<h1 class="pageMainTitle"><?=lang("Products", "منتجات الخيل"); ?></h1>
...
<?php
	$qu_users_ads_sel = "SELECT * FROM  `users_ads` WHERE ((`category_id` IN (2) ) AND (`ad_status` = 'published'))";
...
if( $adMainTitle != "" ){
	...
	echo $thsUserfull_name;
}
?>
			</h1>
		</a>
		<span class="price"><?=$ad_price; ?> AED</span>
		<a href="cart.php">
			<button><?=lang("Add to cart", "اضافة إلى العربة"); ?><i class="fas fa-cart-plus"></i></button>
		</a>
	</div>
...
?>
	...
	<a href="add_adv.php?typer=2&sub=4&extra_l=0" class="product producterHolder">

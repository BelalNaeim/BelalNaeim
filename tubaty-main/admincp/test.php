...
abstract class MysqliConnect implements DatabaseConnect {
    ...
    public function update( $table, $data, $colum, $id, $other = null ) {
        $this->stmt = mysqli_query( $this->conn(), "UPDATE `{$table}` SET {$data} WHERE `{$colum}` = ' {
                $id}
                ' {$other}" );
    }

    public function delete( $table, $colum, $id, $other = null ) {
        $this->stmt = mysqli_query( $this->conn(), "DELETE FROM `{$table}` WHERE `{$colum}` = ' {
                    $id}
                    ' {
                        $other}
                        " );
...
class Database extends MysqliConnect {
